...
public class Sha256SaltAuth
{
    private final static int ITERATION_NUMBER = 1000;

    public Sha256SaltAuth() {
    }
    ...
    public boolean createUser(Connection con, String login, String password)
            throws SQLException, NoSuchAlgorithmException, InvalidArgumentException {
        if(login == null || password == null)
            return false;

        if(login.length() > 255)
            throw new InvalidArgumentException(new String[]{"Login possui mais de 255 caracteres"});
        if(password.length() < 6  || !password.matches(".*\\d.*") || !password.matches(".*[A-Z].*") || !password.matches(".*[a-z].*") || password.matches("[a-zA-Z0-9 ]*"))
            throw new InvalidArgumentException(new String[]{"Senha fraca"});
